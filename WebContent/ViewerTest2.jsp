...
<div id="container"></div>
	<script>		
		...
		function matrixfile(){
			...
			matrix_xhr.open("get", "txt/abcf.csv", true);
...
/* 			if ( mesh !== undefined ) {
			...
			visual_first = performance.now(); //시작시간 체크(단위 ms)
			...
			alert(matrix_array.length);
			
			/* 	var random_first = performance.now();     //시작시간 체크(단위 ms) */

			lhc_array_len = lhc_array.length;

			for (var i = 0; i < 210; i++) { //lh_c길이 만큼 난수 생성 
				 random_array[i] = matrix_array[input][i] + 1;  // 마이너스를 양수로 처리.
