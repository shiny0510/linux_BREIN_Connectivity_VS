...
lhc_xhr.open("get", "txt/lh_c2.txt", true); //c에서 RGB 제거한 값
...
lhlabel_xhr.open("get", "txt/lh_label.txt", true); //label 읽는거
     ...
     for(var i=0; i < lhlabel_rowData.length; i++){
    	 ...
    	 var lhlabel_colData = lhlabel_rowData[i].split(" "); //열은 ,로 구분
          ...
          for(var j=0; j < lhlabel_colData.length; j++){
        	  lhlabel_array[j] = parseInt(lhlabel_colData[j]);
          }
    	 }
}
/* ■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■   lh_a ■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■*/
...
var lha_xhr;
lha_xhr = new XMLHttpRequest();
lha_xhr.open("get", "txt/lh_a.txt", true);
lha_xhr.onreadystatechange = function(){
          if(lha_xhr.readyState == 4){
              if(lha_xhr.status== 200){
            	  lha_process();
              }else{
                   alert("요청 실패: " + lha_xhr.status);
              }    
          }
     }
lha_xhr.send(null);

function lha_process(){
     lha_datas = lha_xhr.responseText;
     
     lha_rowData = lha_datas.split(String.fromCharCode(13)); //행은 cr로 구분 (String.fromCharCode(13))
     
     lha_array = new Array();
     str ="";
     for(var i=0; i < lha_rowData.length; i++){
    	 lha_array[i] = new Array(); 
    	 lha_colData = lha_rowData[i].split(","); //열은 ,로 구분
     
          for(var j=0; j < lha_colData.length; j++){
        	  lha_array[i][j] = String(lha_colData[j]);
          }
    	 }
}
...
obj_xhr.open("get", "assets/lhmesh.obj", true);
...
function obj_process(){
	...
	obj_rowData = obj_datas.split(String.fromCharCode(13)); //행은 cr로 구분 (String.fromCharCode(13))
